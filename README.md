...
Ключевые документы проекта находятся в папке `docs/`:
- `docs/prd.md` — постановка задачи.
- `docs/architecture.md` — архитектура решения и план развития.
- `docs/structure.md` — структура проекта и зоны ответственности.
...
1. Создайте папки в корневой директории (если их еще нет):
...
> Примечание: для запуска тестов или работы с ноутбуками,
> проще сразу настроить локальную среду (`uv venv` + `uv sync` с нужной группой зависимостей),
> чем использовать Docker. Docker в этом проекте в первую очередь удобен для воспроизводимого
> запуска приложения.
...
Команда установит:
- базовые зависимости из `[project].dependencies`;
- OCR-зависимости из `[dependency-groups].ocr` (`paddleocr`, `paddlepaddle`).
